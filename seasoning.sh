...
usage:
  ...
  seasoning ctl mid   {cycle|prev|set N|mode-next|mode-sync}   [--output NAME]
  seasoning ctl sides {cycle|prev|set N|mode-next|mode-sync}   [--which left|right] [--output NAME]
...
which-output() {
  ...
  # Cache the detection method to avoid repeated command lookups
  if [[ -z "${_OUTPUT_METHOD:-}" ]]; then
    if command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
      _OUTPUT_METHOD="hyprctl"
    elif command -v swaymsg >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
      _OUTPUT_METHOD="swaymsg"
    else
      _OUTPUT_METHOD="default"
    fi
  fi
  
  case "$_OUTPUT_METHOD" in
    hyprctl) 
      hyprctl monitors -j | jq -r '.[]|select(.focused==true or .active==true)|.name' | head -n1 
      ;;
    swaymsg) 
      swaymsg -r -t get_outputs | jq -r '.[]|select(.focused==true)|.name' | head -n1 
      ;;
    *) 
      printf '%s\n' default 
      ;;
...
run_mid(){
  ...
  if (( ${#arr[@]} == 0 )); then 
    echo '{"text":"[seasoning:mid:no-plugins]"}'
    return 0
  fi
  
  local f idx p ret
  f="$(_state_path "$out" mid)"
  idx="$(_read_int "$f")"
  (( idx = idx % ${#arr[@]} ))
  p="${arr[$idx]}"
...
ctl_mid(){
  ...
  case "$action" in
    ...
    set)
      ...
      ;;
    mode-next)
      # Find active plugin basename and bump its mode file
      f="$(_state_path "$out" mid)"
      cur="$(_read_int "$f")"
      (( cur = cur % ${#arr[@]} ))
      local base="${arr[$cur]##*/}" mf
      mf="$(_state_path "$out" "mode.$base")"
      _write_int "$mf" $(( $(_read_int "$mf") + 1 ))
      pkill -x -RTMIN+6 waybar 2>/dev/null || true
      ;;
    mode-sync)
      # Sync current mid plugin's mode across all monitors
      f="$(_state_path "$out" mid)"
      cur="$(_read_int "$f")"
      (( cur = cur % ${#arr[@]} ))
      local base="${arr[$cur]##*/}"
      local src="$(_state_path "$out" "mode.$base")"
      local val="$(_read_int "$src")"
